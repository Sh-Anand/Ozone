...
static const size_t CHILD_BLOCK_SIZE[] = { VMSAv8_64_L0_SIZE, VMSAv8_64_L1_BLOCK_SIZE, VMSAv8_64_L2_BLOCK_SIZE,
                                          VMSAv8_64_BASE_PAGE_SIZE };
static const size_t CHILD_PAGE_TABLE_TYPE[] = { ObjType_VNode_AARCH64_l1, ObjType_VNode_AARCH64_l2,
...
static errval_t create_node(struct paging_state *st, size_t index, size_t count, struct vnode_cap_node **ret)
{
...
static errval_t apply_mapping(struct paging_state *st, struct capref dest, struct capref src, capaddr_t slot,
...
static errval_t upgrade_node_to_page_table(struct paging_state *st, struct vnode_cap_node *n, enum objtype type,
                                          struct capref parent_cap)
{
   ...
   err = apply_mapping(st, parent_cap, n->cap, n->index, KPI_PAGING_FLAGS_READ | KPI_PAGING_FLAGS_WRITE, 0, 1);
   if (err_is_fail(err)) {
       return err;
   }
   return SYS_ERR_OK;
...
*/
errval_t paging_init_state(struct paging_state *st, lvaddr_t start_vaddr, struct capref pdir, struct slot_allocator *ca)
{
...
* indicating what went wrong otherwise.
*/
errval_t paging_init_state_foreign(struct paging_state *st, lvaddr_t start_vaddr, struct capref pdir,
...
errval_t paging_init(void)
{
   ...
   errval_t err;
   err = paging_init_state(&current, VMSAv8_64_L0_SIZE, cap_vroot, get_default_slot_allocator());
   if (err_is_fail(err)) {
       return err;
   }
...
static void update_max_continuous_count(struct vnode_cap_node *node)
{
   if (LIST_EMPTY(&node->children)) {
       ...
       {
           size_t next_index = (LIST_NEXT(n, link) == NULL ? VMSAv8_64_PTABLE_NUM_ENTRIES : LIST_NEXT(n, link)->index);
...
*       including recursion.
*/
static errval_t map_recursive(struct paging_state *st, struct vnode_cap_node *node, size_t level, lvaddr_t rvaddr,
                             struct capref frame, size_t offset, size_t bytes, uint64_t attr, lvaddr_t *out_rvaddr)
{
//    debug_printf("map_recursive level = %u, rvaddr = %lx, bytes = %lu, out_rvaddr = %p\n", level, rvaddr, bytes, out_rvaddr);
   ...
   size_t child_end_index = get_child_index(ROUND_UP(rvaddr + bytes, CHILD_SIZE), level);  // exclusive
   if (child_end_index == 0) child_end_index = 512;
       ...
       LIST_FOREACH(n, &node->children, link)
       {
           ...
           if (out_rvaddr == NULL) {
               if (n->index == child_start_index) {
                   ...
                   err = map_recursive(st, n, level + 1, rvaddr - n->index * CHILD_SIZE, frame, offset, bytes, attr, out_rvaddr);
                   return err;  // must be this node, return directly, either work or fail
               } else if (n->index > child_start_index) {
                   break;
               }
           } else {
               err = map_recursive(st, n, level + 1, 0 /* useless */, frame, offset, bytes, attr, out_rvaddr);
   ...
   struct vnode_cap_node *n;
   struct vnode_cap_node *heading = NULL;
   ...
   if (LIST_EMPTY(&node->children)) {
       ...
       LIST_FOREACH(n, &node->children, link)
       {
           ...
           assert(continuous_count <= node->max_continuous_count && "paging: max_continuous_count too small");
           ...
           if (out_rvaddr == NULL) {  // require specific mapping
               ...
               if (n->index + n->count <= child_start_index + 1 && next_index >= child_end_index) {
                   should_update_max_continuous_count = (continuous_count == node->max_continuous_count);
               ...
               if (continuous_count >= child_count) {
                   ...
                   rvaddr = child_start_index * CHILD_SIZE;  // the map size computation below will just work
                   break;
               }
           }
       }
   ...
   if (level < 3) {
       ...
       // This should never trigger a refill since ensure_enough_slabs() is expected before every call in this function
       err = create_node(st, child_start_index +  (heading != NULL), child_count - (heading != NULL), &placeholder);
       if (err_is_fail(err)) {
           return err;
       }
       ...
       for (size_t i = child_start_index; i < child_end_index; i++) {
           ...
           if (heading != NULL) {
               ...
               err = upgrade_node_to_page_table(st, new_node, CHILD_PAGE_TABLE_TYPE[level], node->cap);
               if (err_is_fail(err)) {
                   return err;
               }
           }
           ...
           err = map_recursive(st, new_node, level + 1, next_rvaddr, frame, offset, child_mapping_size, attr,
                               out_rvaddr);
       ...
       err = apply_mapping(st, node->cap, frame, child_start_index, attr, offset, child_count);
       if (err_is_fail(err)) {
           return err;
       }
...
* otherwise.
*/
errval_t paging_map_frame_attr(struct paging_state *st, void **buf, size_t bytes, struct capref frame, int flags)
{
   // TODO: allocated nodes and slots are not freed on failure return

   assert(st->l0 != NULL);

   if (bytes >= VMSAv8_64_L0_SIZE * VMSAv8_64_PTABLE_NUM_ENTRIES) {  // trying to map into kernel
       return MM_ERR_NOT_FOUND;
   }
   if (bytes == 0) {
       return SYS_ERR_OK;
   ...
   return map_recursive(st, st->l0, 0, 0, frame, 0, ROUND_UP(bytes, BASE_PAGE_SIZE), flags_to_attr(flags), (lvaddr_t *)buf);
}
...
*/
errval_t paging_map_fixed_attr(struct paging_state *st, lvaddr_t vaddr, struct capref frame, size_t bytes, int flags)
{
   // TODO: allocated nodes and slots are not freed on failure return
   ...
   if (vaddr >= VMSAv8_64_L0_SIZE * VMSAv8_64_PTABLE_NUM_ENTRIES /* plus can overflow */ ||
       vaddr + bytes >= VMSAv8_64_L0_SIZE * VMSAv8_64_PTABLE_NUM_ENTRIES) {  // map into kernel address space
       return MM_ERR_NOT_FOUND;
   }
   if (bytes == 0) {
       return SYS_ERR_OK;
   ...
   return map_recursive(st, st->l0, 0, vaddr, frame, 0, ROUND_UP(bytes, BASE_PAGE_SIZE), flags_to_attr(flags), NULL);
}
