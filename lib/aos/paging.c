	...
	for (int i = start; i <= end; i++) {
		...
		if (current_meta == NULL || current_meta->entry.slot != i) {
			...
			if (depth < 3) {
				...
				off = frame_offset + i * sub_region_size; // pages have a necessary frame offset if more than a single page is mapped
			}
		...
		if (depth < 3) {
			...
			err = rec_map_fixed(st, &current_meta->vnode, i == start ? rvaddr % sub_region_size : 0, next_size, frame, frame_offset + (i - start) * sub_region_size, flags, depth + 1);
			if (err_is_fail(err)) {
